---
title: "Vogt_Johnny_Unit3_Prelive"
author: "Johnny Vogt"
date: "`r Sys.Date()`"
...
"i" is the index vector. It acts as a way to help  choose the "median" only from the dataset that its been given. so it would have to pick the median between 18 10 16 and 14
...
sd.fun <- function(x, i) sd(x[i])
iqr.fun <- function(x, i) IQR(x[i])

sd.boot <- boot(data, sd.fun, R=2000)
iqr.boot <- boot(data, iqr.fun, R=2000)

boot.ci(sd.boot, conf=0.95, type=c("perc","basic","bca"))
boot.ci(iqr.boot, conf=0.95, type=c("perc","basic","bca"))
...
plot(x,log(y),main="logged response sample")
...
I would prefer using the  Paired Bootstrap because it handles variances changing. Since this data set's does not have a strong homoscedasticity (the data get narrower as x grows.)
...
The intervals get narrower with the paired bootstrap. I would choose the paired bootstrap in this case. 
