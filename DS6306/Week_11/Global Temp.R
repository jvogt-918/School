...
#For Global Temp CSV
maxtemp = read.csv(file.choose(), header = TRUE)

# Number 1: In this exercise, we will use Temperature data
maxtemp_ts = ts(maxtemp$Actual, frequency = 1, start = 1880, end = 2023)
autoplot(maxtemp_ts, xlab="Year", ylab="Temperature (Celsius)")
...
ts2<-window(maxtemp_ts, start=1990, end = 2018)
fit2<-ses(ts2, alpha = .05, h=5)
accuracy(fit2,ts)

plot(fit2,ylab="Temperature, Celsius", xlab= "Year", main="Comparing forecast fit",ylim = c(.1,1.3))
...
#External CV
ts2<-window(maxtemp, start=1990, end = 2018)
holtfit2<- holt(ts2, initial="optimal", h=11, damped = TRUE)
...
plot(holtfit,ylab="Temperature, Celsius", xlab= "Year", main="Comparing forecast fit",ylim = c(30,50))
